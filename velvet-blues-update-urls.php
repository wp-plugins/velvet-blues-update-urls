...
function VelvetBluesUU_options_page(){
	...
	elseif( isset( $_POST['VBUU_settings_submit'] ) ){
		...
		if(($vbuu_oldurl && $vbuu_oldurl != 'http://www.oldurl.com' && trim($vbuu_oldurl) != '') && ($vbuu_newurl && $vbuu_newurl != 'http://www.newurl.com' && trim($vbuu_newurl) != '')){
			...
			$empty = false;
			$emptystring = '<b>'.__('Why do the results show 0 URLs updated?','velvet-blues-update-urls').'</b><br/>'.__('This happens if a URL is incorrect or if it is not found in the content.','velvet-blues-update-urls');
			$resultstring = '';
			foreach($results as $result){
				$empty = ($result[0] == 0 || $empty == true)? true : false;
				$resultstring .= '<br/><b>'.$result[0].'</b> '.$result[1];
			}
			?>
...
<p><?php printf(__("After moving a website, %s lets you fix old URLs in content, excerpts, and custom fields.",'velvet-blues-update-urls'),'<b>Update URLs</b>'); ?></p>
...
<th scope="row" width="140" style="width:140px"><b><?php _e('Old URL','velvet-blues-update-urls'); ?></b><br/><span class="description"><?php _e('Old Site Address','velvet-blues-update-urls'); ?></span></th>
...
<tr valign="middle">
<th scope="row" width="140" style="width:140px"><b><?php _e('New URL','velvet-blues-update-urls'); ?></b><br/><span class="description"><?php _e('New Site Address','velvet-blues-update-urls'); ?></span></th>
...
<input name="VBUU_update_links[]" type="checkbox" id="VBUU_update_true" value="content" checked="checked" /> <label for="VBUU_update_true"><b><?php _e('URLs in page content','velvet-blues-update-urls'); ?></b> (<?php _e('posts, pages, custom post types, revisions','velvet-blues-update-urls'); ?>)</label><br/>
<input name="VBUU_update_links[]" type="checkbox" id="VBUU_update_true" value="excerpts" /> <label for="VBUU_update_true"><b><?php _e('URLs in excerpts','velvet-blues-update-urls'); ?></b></label><br/>
<input name="VBUU_update_links[]" type="checkbox" id="VBUU_update_true" value="attachments" /> <label for="VBUU_update_true"><b><?php _e('URLs for attachments','velvet-blues-update-urls'); ?></b> (<?php _e('images, documents, general media','velvet-blues-update-urls'); ?>)</label><br/>
<input name="VBUU_update_links[]" type="checkbox" id="VBUU_update_true" value="custom" /> <label for="VBUU_update_true"><b><?php _e('URLs in custom fields and meta boxes','velvet-blues-update-urls'); ?></b></label><br/>
<input name="VBUU_update_links[]" type="checkbox" id="VBUU_update_true" value="guids" /> <label for="VBUU_update_true"><b><?php _e('Update ALL GUIDs','velvet-blues-update-urls'); ?></b> <span class="description" style="color:#f00;"><?php _e('GUIDs for posts should only be changed on development sites.','velvet-blues-update-urls'); ?></span> <a href="http://www.velvetblues.com/go/guids/" target="_blank"><?php _e('Learn More.','velvet-blues-update-urls'); ?></a></label>
</p></td></tr></table>
...
<p>&nbsp;<br/><?php printf(__("Need help? Get support at the %s.",'velvet-blues-update-urls'),'<a href="http://www.velvetblues.com/web-development-blog/wordpress-plugin-update-urls/" target="_blank">Velvet Blues Update URLs '.__('plugin page','velvet-blues-update-urls').'</a>'); ?><br/><?php _e('Want to donate?','velvet-blues-update-urls'); ?> <a href="http://www.velvetblues.com/go/updateurlsdonate/" target="_blank"><?php _e('Click here.','velvet-blues-update-urls'); ?></a></p>
<?php
}
